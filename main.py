...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user is None:
        ...
        private_key = base58.b58encode(keypair.secret() + bytes(public_key)).decode("utf-8")

        cursor.execute(f"INSERT INTO users (id, pub_key, priv_key) VALUES (?, ?, ?)", (chat_id, public_key, private_key))
        conexion.commit()
